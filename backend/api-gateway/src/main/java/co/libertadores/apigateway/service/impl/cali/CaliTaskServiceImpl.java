...
@Service
@Qualifier("cali")
public class CaliTaskServiceImpl implements ITaskService {
    ...
    public CaliTaskServiceImpl(@Qualifier("cali") IUserService caliUserService, @Value("${cali.task.service.url}") String taskService, RestTemplate restTemplate) {
    ...
    public TaskCreateUpdateResponse save(TaskCreateRequest taskCreateRequest) {
        if (caliUserService.existsById(taskCreateRequest.getUserId()))
    ...
    public TaskCreateUpdateResponse update(Long id, TaskUpdateRequest taskUpdateRequest) {
        ...
        if (caliUserService.existsById(taskUpdateRequest.getUserId()))
            throw new UserException(USER_DOES_NOT_EXISTS);
